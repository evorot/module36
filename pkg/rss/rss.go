	...
	for i, r := range conf.Rss {
		go func(r string, i int, chP chan<- []storage.Post, chE chan<- error) {
			for {
				...
				chP <- p
				log.Println("вствил посты из горутины", i, "по ссылке", r)
				log.Println("отдыхаю, жду следующей итерации")
